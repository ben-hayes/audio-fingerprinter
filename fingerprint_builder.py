...
def pick_peaks(spectrogram, tau=32, kappa=32, hop=64):
    # create empty array to store peaks
    peaks = np.zeros_like(spectrogram)

    # calculate how many hops we will make along each axis
    n_freq_steps = int(np.floor((spectrogram.shape[0] - 2 * kappa) / hop))
    n_time_steps = int(np.floor((spectrogram.shape[1] - 2 * tau) / hop))

    for n in range(n_time_steps):
        for k in range(n_freq_steps):
            # calculate the bounds of our window
            freq_lower = k * hop
            freq_upper = k * hop + 2 * kappa
            time_lower = n * hop
            time_upper = n * hop + 2 * tau
...
def quantise_frequency(peaks, factor=16):
...
def create_inverted_lists(peaks):
    ...
    for row in peaks:
        ...
        inverted_lists.append(inverted_list[0].tolist())
